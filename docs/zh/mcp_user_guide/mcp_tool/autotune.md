...
KernelGen MCP Server 集成了用于自动调优 Kernel 的 MCP 工具。您只需提供算子的功能描述，KernelGen 即可通过多轮智能迭代，自动生成、验证并持续优化算子实现，最终输出高性能 Triton Kernel 及完整的测试报告。
...
**前提条件**：请先配置 KernelGen Server，参阅[配置并连接 KernelGen MCP Server](../connect_mcp/connect-mcp.md)。
